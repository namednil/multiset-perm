...
@DatasetReader.register("seq2seq_with_cat")
class Seq2SeqDatasetReaderWithCat(DatasetReader):
    """
    ...
    def _read(self, file_path: str):
        ...
        with open(cached_path(file_path), "r") as data_file:
            ...
            for line_num, row in self.shard_iterable(
                ...
                source_sequence, target_sequence, category = row
                if len(source_sequence) == 0 or len(target_sequence) == 0:
                    continue
                yield self.text_to_instance(source_sequence, target_sequence, category)
    ...
    def text_to_instance(
        ...
        metadata = {"category": category, "source_text": source_string}
        d = {"source_tokens": source_field }
        if target_string is not None:
            tokenized_target = self._target_tokenizer.tokenize(target_string)
            metadata["target_text"] = target_string
