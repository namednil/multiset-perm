...
def extract_freqs(tensor, vocab_size: int):
    device = tensor.device
    tensor = tensor.cpu().numpy()
    batch_size = tensor.shape[0]
    freqs = np.zeros((batch_size, vocab_size-2), dtype=np.int64) # -2 because AllenNLP adds UNK and PADDING tokens to the "tokens" namespace :(
    for b in range(batch_size):
        in_multiset=False
        freqs[b,0] = True # for the start token?
        for i in tensor[b]:
            if i == 2:
                in_multiset = True
            elif i != 1 and in_multiset: # 1 is padding
                freqs[b, i] += 1
    return torch.from_numpy(freqs).to(device)

@Model.register("constrained_perm_bart")
class PermBartWithMetrics(Model):
    """
    ...
    def forward(
        ...
        # extract the multiset after the </s> token (id 2)
        freqs = extract_freqs(input_ids, self.vocab.get_vocab_size())
        ...
        if self.training:
            ...
            # ~ print(predictions.unsqueeze(1).shape, target_ids.shape)

            # ~ self._seq_acc(predictions.unsqueeze(1), target_ids, target_mask)
            ...
            gold_text = self._indexer._tokenizer.batch_decode(target_ids.tolist(), skip_special_tokens=True)
            for m in self.metrics:
                m.add_instances(outputs["predicted_text"], gold_text)

            if metadata is not None:
                categories = [m["category"] if "category" in m else None for m in metadata]
                self.track_metric_by_cat.add_instances(outputs["predicted_text"], gold_text, categories)
    ...
    def take_step(
        ...
        """
        if len(last_predictions.shape) == 2:
            sq_last_pred = last_predictions.squeeze(1)
        else:
            sq_last_pred = last_predictions
        ...
        assert torch.all(sq_last_pred < freqs.shape[-1])
        freqs[range(sq_last_pred.shape[0]), sq_last_pred] -= 1
        freqs[:, self._end_id] = 0
        ...
        decoder_cache_dict = {
            ...
            for k in state
            if k not in {"input_ids", "input_mask", "encoder_states", "freqs"}
        }
        ...
        if self.constrained_decoding:
            have_to_stop_now = freqs.sum(dim=-1) == 0
            end_mask = torch.zeros_like(freqs, dtype=torch.bool)
            end_mask[:, self._end_id] = True

            # mask out impossible things
            m = (freqs <= 0)
            m[:, self._end_id] = True  # mask out end symbol, we will mask it in, if necessary

            logits = logits - 10_000 * m + 100_000 * end_mask * have_to_stop_now.unsqueeze(-1)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        metrics: Dict[str, float] = self.track_metric_by_cat.get_metrics(reset)
        for m in self.metrics:
            metrics.update(m.get_metric(reset))
        return metrics
