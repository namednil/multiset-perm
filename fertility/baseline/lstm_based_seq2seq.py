...
# This is the LSTM-based sequence to sequence model from allennlp-models.
from fertility.baseline.grammar_decoding import GrammarFinalSequenceScorer
...
class LSTMSeq2Seq(Model):
    ...
    def __init__(
        ...
        grammar_decoder: Optional[Lazy[DecodingGrammar]] = None,
        test_beam_size: int = 50,
        **kwargs
    ) -> None:
        ...
        test_beam_search_extras = dict(beam_search_extras)
        if grammar_decoder:
            test_beam_search_extras["final_sequence_scorer"] = GrammarFinalSequenceScorer(grammar_decoder.construct(tok2id=self.vocab.get_token_to_index_vocabulary(self._target_namespace)))
        self._test_beam_search = copy.deepcopy(beam_search).construct(end_index=self._end_index, vocab=self.vocab, **test_beam_search_extras)
        self._test_beam_search.beam_size = test_beam_size
    ...
    def forward(
        ...
        if not self.training:
            state = self._init_decoder_state(state)
            predictions = self._forward_beam_search(state, self._beam_search)
            ...
            predicted_lengths = best_predictions.shape[1] - (best_predictions == self._end_index).sum(dim=-1)
            readable_pred = self.make_readable(best_predictions, predicted_lengths, self._target_namespace)
            if target_tokens:
                readable_gold = [m["target_tokens"] for m in metadata]

                for m in self.metrics:
                    m.add_instances(readable_pred, readable_gold)


            if self.test_mode.is_in_test_mode(self):
                state = self._encode(source_tokens)
                state = self._init_decoder_state(state)
                predictions = self._forward_beam_search(state, self._test_beam_search)
                output_dict.update({"test_mode_"+k: v for k,v in predictions.items()})
                # shape: (batch_size, beam_size, max_sequence_length)
                top_k_predictions = output_dict["test_mode_predictions"]
                # shape: (batch_size, max_predicted_sequence_length)
                best_predictions = top_k_predictions[:, 0, :]
                predicted_lengths = best_predictions.shape[1] - (best_predictions == self._end_index).sum(dim=-1)
                readable_pred = self.make_readable(best_predictions, predicted_lengths, self._target_namespace)
                ...
                if target_tokens:
                    assert len(readable_pred) == len(readable_gold)
                    self.test_mode.add_instances(readable_pred, readable_gold)
    ...
    def _forward_beam_search(self, state: Dict[str, torch.Tensor], beam_search) -> Dict[str, torch.Tensor]:
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if not self.training:
            for m in self.metrics:
                all_metrics.update(m.get_metric(reset))

            if self.test_mode.is_in_test_mode(self):
                all_metrics.update(self.test_mode.get_metrics(reset))
