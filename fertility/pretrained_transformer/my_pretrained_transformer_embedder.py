...
@TokenEmbedder.register("pretrained_transformer_rho")
class PretrainedTransformerEmbedderWithRho(TokenEmbedder):
    """
    ...
    def __init__(
        ...
        not_adding_positional: bool = False,
        *,
        max_length: int = None,
        sub_module: str = None,
        ...
        self.not_adding_positional = not_adding_positional
    ...
    def forward(
        ...
        if self.not_adding_positional:
            word_rep = self.transformer_model.get_input_embeddings()(token_ids)
        else:
            # Includes positional embedding
            word_rep = transformer_output.hidden_states[0]

        #Focus on static word embedding
        # embeddings = self.rho * embeddings + transformer_output.hidden_states[0]
        if self.train_rho:
            embeddings = torch.sigmoid(self.rho) * embeddings + word_rep
        else:
            embeddings = self.static_rho * embeddings + word_rep
        # embeddings = transformer_output.hidden_states[0]
        # print(torch.sigmoid(self.rho))
